					...
					<tbody>
						...
						foreach( $wpsp_exams as $wpsp_exam ) {
							...
							if( !empty($wpsp_exam->subject_id) ) {
								...
								if( !empty( $slist ) ) {											
									$subjectlist	=	$wpdb->get_results("SELECT sub_name FROM $subject_table WHERE id IN($slist) and sub_teach_id = $cuserId", ARRAY_A );
							...
							<tr id="<?php echo intval($wpsp_exam->eid);?>">
								...
								<td><?php echo  wpsp_ViewDate($wpsp_exam->e_e_date);?></td>
									...
									<div class="wpsp-action-col">
										...
										<!-- <a href="javascript:;" eid="<?php echo intval($wpsp_exam->eid);?>" class="ExamDeleteBt" title="Delete"><i class="icon dashicons dashicons-trash wpsp-delete-icon"></i></a>	 -->
									</div>
								</td>
							<?php } ?> 
							</tr>
